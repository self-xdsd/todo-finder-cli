...
public class TodoParserTest {
    ...
    public void whenParsingRtClass_ExpectTwoReturnedTodos() throws IOException {
        List<Todo> todos = TodoParser.parse("src/test/resources/RtImagesITCase.java");
    ...
    public void whenParsingLegalTodosClass_ExpectExactlyTenTodos() throws IOException {
        List<Todo> todos = TodoParser.parse("src/test/resources/LegalTodos.java");
        ...
        assertEquals(new Todo(20, 34, "#113", "13min"), todos.get(3));
        assertEquals(new Todo(40, 40, "#114", "14min"), todos.get(4));
        assertEquals(new Todo(41, 41, "#115", "15min"), todos.get(5));
        assertEquals(new Todo(49, 51, "#116", "16min"), todos.get(6));
        assertEquals(new Todo(61, 61, "#117", "17min"), todos.get(7));
        assertEquals(new Todo(62, 63, "#118", "18min"), todos.get(8));
        assertEquals(new Todo(64, 64, "#119", "19min"), todos.get(9));
    }

    @Test
    public void whenParsingConfigurationPropertiesClass_ExpectNoTodos() throws IOException {
        List<Todo> todos = TodoParser.parse("src/test/resources/ConfigurationPropertiesReportEndpoint.java");
        assertEquals(0, todos.size());
    }

    @Test
    public void whenParsingHashMapClass_ExpectNoTodos() throws IOException {
        List<Todo> todos = TodoParser.parse("src/test/resources/HashMap.java");
        assertEquals(0, todos.size());
    }

    @Test
    public void whenParsingAbstractStringBuilder_ExpectNoTodos() throws IOException {
        List<Todo> todos = TodoParser.parse("src/test/resources/AbstractStringBuilder.java");
        assertEquals(0, todos.size());
    }

    @Test()
    public void whenParsingIllegalTodoClass_ExpectIllegalTodoException() throws IOException {
        List<Todo> todos = TodoParser.parse("src/test/resources/IllegalTodos.java");
        assertEquals(0, todos.size());
    }

    @Test
    public void whenParsingPhonyClass_ExpectNoTodos() throws IOException {
        List<Todo> todos = TodoParser.parse("src/test/resources/PhonyClass.java");
        assertEquals(0, todos.size());
    }
}